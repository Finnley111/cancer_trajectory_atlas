...
#SBATCH --mem=64G
#SBATCH --job-name=atlas_reinhard
#SBATCH --output=logs/atlas_reinhard-%j.out

mkdir -p logs
mkdir -p $SCRATCH/results/atlas_full_reinhard
...
python -m cancer_trajectory_atlas.run_all \
    ...
    --output-dir        $SCRATCH/results/atlas_full_reinhard \
    --stain-method      reinhard \
    --model             phikon \
    --patch-size        112 \
    --stride            96 \
...
du -sh $SCRATCH/results/atlas_full_reinhard 2>/dev/null
