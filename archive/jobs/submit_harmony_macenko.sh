#!/bin/bash
# SLURM job script — Cancer Trajectory Atlas: Macenko stain normalization + Harmony
#
# Usage:
#   sbatch submit_harmony_macenko.sh               # default: section_number key
#   sbatch submit_harmony_macenko.sh slide_id      # ablation: 16-group correction
#   sbatch submit_harmony_macenko.sh mouse_id
#
...
#SBATCH --mem=128G
#SBATCH --gres=gpu:1
#SBATCH --job-name=atlas_macenko_harmony
#SBATCH --output=logs/atlas_macenko_harmony-%j.out
...
if [ "$HARMONY_KEY" = "section_number" ]; then
    OUT_NAME="atlas_macenko_harmony"
else
    OUT_NAME="atlas_macenko_harmony_${HARMONY_KEY}"
fi
