...
There are two annotation directories under `~/cancer_trajectory_atlas/data/`:

- `annotations/` — raw QuPath GeoJSON exports (original coordinate space from QuPath)
- `annotations_ratio/` — ratio-coordinate JSON files (converted by `converters/geojson_to_ratio_json.py`)

**Always use `annotations_ratio/` for pipeline runs.** All job scripts and `paths.json`
point here. The ratio format is what `load_roi_polygons()` has been tested with.

The `annotations/` directory is an archive. Do not use it as the active annotation source.
